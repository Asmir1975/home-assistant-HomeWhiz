    ...
    async def async_step_bluetooth_connect(
        ...
        if user_input is not None:
            ...
            try:
                ...
                # Tuple unpacking - api.py returns (ApplianceContents, bool)
                contents, api_config_missing = await fetch_appliance_contents(
                ...
                # Defense in Depth: Validate with local check AND API flag
                local_check_missing = not has_real_config(contents)
                config_missing = api_config_missing or local_check_missing

                if config_missing:
                    _LOGGER.warning(
                        "Device %s has no config - using functions (Issue #295) [API=%s, Local=%s]",
                        id_response.appId,
                        api_config_missing,
                        local_check_missing,
                    )
    ...
    async def async_step_select_cloud_device(
        ...
        if user_input is not None:
            ...
            try:
                ...
                # This should now be rare - fetch_appliance_contents returns minimal config instead of raising
                _LOGGER.error(
                    "Unexpected RequestError for %s: %s (fetch_appliance_contents should handle this)",
                    appliance_id,
                    err,
                )
                errors["base"] = "cannot_connect"
